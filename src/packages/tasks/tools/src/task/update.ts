...
import { withBatchSupport, runBatchTool, required, locked, type Resolved } from "@dav-worker/batch-core";

// id is required() — every item must name its own target, there's no
// sensible top-level default an id could inherit. `list` here means
// "move to," and stays plain .optional() (undefined = don't move) rather
// than required() — but it's still locked(): letting individual batch
// items move to different destinations without the caller separately
// saying so is the same footgun task_create's `list` guards against.
// A batch either shares one move target (top-level default, or omitted
// entirely to mean "don't move any of them") or the caller makes
// separate calls. Every other field keeps its original
// change-only-what's-provided optionality untouched — required()/
// locked() only apply where this review flagged an actual risk.
const itemShape = {
...
type UpdateItem = Resolved<typeof itemShape, "id">;
...
export function registerTaskUpdateTool(server: McpServer, deps: TaskToolsDeps): void {
  server.registerTool(
    ...
    async (params) =>
      runBatchTool(params, itemShape, err, async ({ id, title, progress, priority, tags, url, event_id, list }: UpdateItem) => {
        // An empty list slug resolves to the calendars home collection
        // itself (davPath(basePath, "") === basePath), not a 404 — same
        // guard as list_create/list_delete/task_create. `list` is
        // optional here (undefined = "don't move"), so only reject an
        // explicit "" — required() isn't the right tag for this field
        // since undefined is a valid, meaningful value, not an omission
        // to reject.
        if (list === "") {
